...
// Использует WithDBTx для безопасной работы с транзакциями без блокировки мьютекса на весь метод
...
func (d *DBConnection) SaveSmsResponse(ctx context.Context, params SaveSmsResponseParams) (bool, error) {
	// Быстрая проверка соединения БЕЗ блокировки мьютекса
	if d.db == nil {
		return false, fmt.Errorf("соединение с БД не открыто")
	}

	// Проверяем соединение БЕЗ блокировки мьютекса
	...
	// Это предотвращает переподключение во время транзакции и не блокирует мьютекс на весь метод
