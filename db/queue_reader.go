...
func (qr *QueueReader) DequeueMany(ctx context.Context, count int) ([]*QueueMessage, error) {
	...
	// Проверка db == nil не нужна, так как wrapper методы (WithDB, WithDBTx) сами проверят это
	if count <= 0 {
		count = 1
	}
...
func (qr *QueueReader) ensurePackageExists(ctx context.Context) error {
	...
	// Используем WithDB для безопасного доступа к пулу соединений
	// Это защищает от race condition при переподключении
...
func (qr *QueueReader) dequeueOneMessageWithTimeout(ctx context.Context, timeout float64) (*QueueMessage, error) {
	...
	// Используем WithDBTx для безопасного доступа к транзакции
	// Это защищает от race condition при переподключении
	// WithDBTx автоматически управляет счетчиком активных операций
	...
	err := qr.dbConn.WithDBTx(txCtx, func(tx *sql.Tx) error {
		...
		if !successFlag.Valid || successFlag.Int64 == 0 {
			if errorCode.Valid && errorCode.Int64 == -25228 {
				return nil // Очередь пуста - возвращаем nil
			}
		...
		return nil // Транзакция будет автоматически закоммичена в WithDBTx
	})
	...
	// Если msg == nil, значит очередь пуста
	return msg, nil
}
